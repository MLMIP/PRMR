...
class DenoisingModel(nn.Module):
    ...
    def __init__(self, diffusion: DiffusionModel, unet: nn.Module, guidance_scale: float, guidance_scale_weighting: str,
                 guidance_loss_fn: str, step_T_sample:str = "majority"):
        super().__init__()
    ...
    def forward(self, x: Tensor,
        ...
        if self.training:
            ...
            return self.forward_step(x, t, get_multiscale_predictions=get_multiscale_predictions)  # returns dictionary with {"diffusion_out":tensor, ...}
        else:
            if validation:
                return self.forward_step(x, t)
            if t is None:
                print("sample")
                return self.forward_denoising(x, label_ref_logits=label_ref_logits)

            return self.forward_denoising(x, cast(int, t.item()), label_ref_logits)

    def forward_step(self, x: Tensor, t: Tensor, get_multiscale_predictions=False) -> Tensor:

        ret = self.unet(x, t, get_multiscale_predictions=get_multiscale_predictions)
        return ret

    def forward_denoising(self, x: Optional[Tensor], init_t: Optional[int] = None,
