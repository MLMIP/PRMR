...
def save_image(tensor: Union[torch.Tensor, List[torch.Tensor]],
    ...
    ndarr = grid.mul(127.5).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
...
@torch.no_grad()
def grid_of_predictions(model: Model, loader: DataLoader, num_predictions: int, params: dict) -> Tensor:
    model.eval()
...
def _onehot_to_color_image(source, params):
    if "cityscapes" in params["dataset_file"]:
        ...
        target = torch.argmax(source, axis=1, keepdim=True).cpu().detach()
    return target
