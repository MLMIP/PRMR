...
# from datasets.fundus import training_dataset
...
class Trainer:
    ...
    def train_step_categorical(self, engine: Engine, batch) -> dict:
        ...
        ret = self.model(xt.contiguous(), t, get_multiscale_predictions=self.use_ms_loss)
    ...
    @torch.no_grad()
    def predict(self, xt: Tensor, label_ref: Optional[Tensor] = None) -> Tensor:
        self.average_model.eval()
        ret = self.average_model(xt)
...
def build_engine(trainer: Trainer,
    ...
    validation_freq = params["validation_freq"] if "validation_freq" in params else 10000
    save_freq = params["save_freq"] if "save_freq" in params else 10000
    display_freq = params['display_freq'] if "display_freq" in params else 1000
    ...
    def save_qualitative_results(_: Engine, num_images=n_validation_images, num_predictions=n_validation_predictions):
        ...
        grid = grid_of_predictions(_flatten(trainer.average_model), loader, num_predictions, params)
        loader = _loader_subset(validation_loader, num_images, randomize=True)
        grid_shuffle = grid_of_predictions(_flatten(trainer.average_model), loader, num_predictions, params)
...
def _build_model(params: dict, input_shapes: List[Tuple[int, int, int]]) -> Model:
...
def _build_datasets(params: dict) -> Tuple[DataLoader, DataLoader, torch.Tensor, int, dict]:
    ...
    # required in params.yml for using datasets/pipelines/transforms
        ...
        validation_dataset = dataset_module.training_dataset()  # type: ignore
...
def run_train(local_rank: int, params: dict):
    ...
    if params['polyak_enabled']:
        ...
        model = _build_model(params, input_shapes)
    ...
    engine = build_engine(trainer, output_path, train_loader, validation_loader,
